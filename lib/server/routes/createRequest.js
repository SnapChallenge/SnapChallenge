        ...
        var requestQuery = {
            $or: [
                ...
                },
                {
                    ...
                    recipient: new ObjectID(request.auth.credentials._id)
                }
            ]
        };

        // ensure a friend request doesn't already exists
        db.request.findOne(requestQuery, function (error, friendReq) {
            ...
            if (friendReq) {
                return reply(Boom.conflict('duplicate friend request'));
            }
