...
yepnope.addPrefix('preload', function (resource) {
    ...
    /*
    var send = document.getElementById('new');
        send.clickTime = null;
        send.addEventListener('click', function() {
            var currentTime = new Date();
            if (currentTime - send.clickTime < 1000) {
                return;
        };
        send.clickTime = currentTime;
        GAME.camera.send();
    });
    ...
    load.addEventListener('click', function() {
        var currentTime = new Date();
        if (currentTime - load.clickTime < 1000) {
            return;
        };
        load.clickTime = currentTime;
        GAME.camera.load();
        console.log('');
    });
    */
