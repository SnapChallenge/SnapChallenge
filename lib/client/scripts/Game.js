GAME.screens['game'] = (function () {
    ...
    function run() {
        ...
        for (var t = 0; t < 100; t++){
            ...
            if (blankIndex - factor >= 0) {
                moves.push(blankIndex - 3);
                console.log('up');
            }

            if (blankIndex + factor < factor * factor) {
                moves.push(blankIndex + 3);
                console.log('down');
            }
            
            if (blankIndex - 1 >= 0 && blankIndex % factor != 0) {
                moves.push(blankIndex - 1);
                console.log('left');
            }

            if (blankIndex + 1 < factor * factor && (blankIndex + 1) % factor != 0) {
                moves.push(blankIndex + 1);
                console.log('right');
            }

            setup(blankIndex, Random.getRandomInteger(moves.length - 1));
        }
